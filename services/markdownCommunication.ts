...
export class MarkdownCommunication {
  ...
  private initializeFiles(): void {
    ...
    if (!fs.existsSync(this.observationFile)) {
      fs.writeFileSync(this.observationFile, `# RAGBOT Web Analyzer - Observations for LLM
...
`);
    ...
    if (!fs.existsSync(this.commandFile)) {
      fs.writeFileSync(this.commandFile, `# RAGBOT Web Analyzer - Commands for Bot
...
`);
  ...
  public writeObservation(observation: any): void {
    ...
    const insertionPoint = content.indexOf('## Observations') + '## Observations'.length;
  ...
  private formatObservationAsMarkdown(observation: WebObservation, timestamp: string): string {
    return `
...
${observation.summary.critical_issues.length > 0 
  ? observation.summary.critical_issues.map(issue => `- **${issue}**`).join('\n')
...
${observation.navigation_guidance.interactive_elements.slice(0, 20).map(el => 
...
### Page Landmarks
${observation.navigation_guidance.page_landmarks.map(landmark => 
  `- **${landmark.type}** - \`${landmark.selector}\`${landmark.label ? ` - "${landmark.label}"` : ''} - ${landmark.description}`
).join('\n')}

### Suggested Actions
${observation.summary.recommended_fixes ? observation.summary.recommended_fixes.slice(0, 10).map(fix => 
...
${observation.accessibility.wcag_compliance.violations.slice(0, 10).map(v => 
...
${observation.accessibility.keyboard_navigation.tab_order_issues.length > 0
  ? observation.accessibility.keyboard_navigation.tab_order_issues.map(issue => `- ${issue}`).join('\n')
  : '✅ No keyboard navigation issues detected'}

### Color Contrast Issues
${observation.accessibility.visual_accessibility.color_contrast.filter(c => !c.passes).slice(0, 5).map(c => 
  `- **\`${c.element}\`** - Ratio: ${c.ratio}:1 (fails WCAG)`
).join('\n')}
...
- **Readability Issues:** ${observation.visual_analysis.typography.readability_issues.length > 0 
...
- **Mixed Content:** ${observation.technical_issues.security.mixed_content.length > 0 
  ? 'Detected' 
  : 'None detected'}
...
${observation.content_analysis.media_content.images.slice(0, 5).map(img => 
  `- \`${img.src}\`${img.alt_text ? ` - "${img.alt_text}"` : ' - **NO ALT TEXT**'}${img.accessibility_issue ? ` (${img.accessibility_issue})` : ''}`
).join('\n')}

### Forms
${observation.ux_assessment.forms.map(form => 
...
`;
  ...
  private parseCommandsFromMarkdown(content: string): any[] {
    ...
    for (let i = 1; i < commandSections.length; i++) {
      ...
      const command: any = {
        ...
        status: status.toLowerCase().includes('pending') ? 'pending' : 'processed',
        raw: section
      };
  ...
  public markCommandProcessed(commandId: string, result?: any): void {
    try {
      ...
      const commandPattern = new RegExp(`(### Command #${commandId} - .*? \\()([^)]*)(\\))`, 'g');
      content = content.replace(commandPattern, (match, prefix, status, suffix) => {
      ...
      if (result) {
        ...
        content = content.slice(0, sectionEnd) + resultMarkdown + content.slice(sectionEnd);
      }
