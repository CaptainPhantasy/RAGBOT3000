import { WebObservation } from './types';
...
export class OutputChannel {
  ...
  private formatForLLM(observation: WebObservation): string {
    return JSON.stringify({
      ...
      navigation_guidance: {
        interactive_elements: observation.navigation_guidance.interactive_elements.map(el => ({
          element: el.element,
          type: el.type,
      ...
      detailed_analysis: {
        ...
        accessibility: {
          ...
          keyboard_issues: observation.accessibility.keyboard_navigation.tab_order_issues,
          contrast_issues: observation.accessibility.visual_accessibility.color_contrast.filter(c => !c.passes)
        },
        css: {
          layout_systems: observation.css_analysis.layout_systems,
          performance_issues: observation.css_analysis.potential_issues.performance_impact,
          responsive: observation.css_analysis.responsive_behavior.breakpoint_analysis
        },
        content: {
          images_without_alt: observation.content_analysis.media_content.images.filter(img => img.accessibility_issue),
  ...
  private generateSuggestedActions(observation: WebObservation): Array<{action: string; target: string; description: string}> {
    const actions = [];
    ...
    if (observation.accessibility.keyboard_navigation.tab_order_issues.length > 0) {
      actions.push({
...
export class InputChannel {
  private handlers: Map<string, (message: CommunicationMessage) => Promise<void>> = new Map();
  
  // Register handlers for different message types
  public registerHandler(type: string, handler: (message: CommunicationMessage) => Promise<void>): void {
  ...
  public parseIncoming(text: string, source: 'llm' | 'human' = 'llm'): CommunicationMessage | null {
    try {
      ...
      if (lowerText.includes('click') || lowerText.includes('navigate') || lowerText.includes('scroll')) {
        type = 'command';
...
export class CommunicationManager implements CommunicationChannel {
  ...
  private async handleAnalysisRequest(message: CommunicationMessage): Promise<void> {
  ...
  private async executeClick(target: string): Promise<any> {
    try {
      ...
      return { error: error instanceof Error ? error.message : 'Unknown error', target };
    }
  }
