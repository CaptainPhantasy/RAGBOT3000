...
import { requestGemini } from "./apiClient";

/**
 * Get greeting message from voice config
 */
export const getGreeting = (
	type: "initial" | "liveStart" | "liveEnd",
): string => {
...
export const analyzeIntent = async (
  ...
  `;
	...
	try {
		const response = await requestGemini<{ text?: string }>("analyzeIntent", {
			prompt,
		});
 ...
 */
const buildGroundedDocsContext = (
	retrievedDocs: DocChunk[],
): { context: string; hasKBSupport: boolean } => {
 ...
 */
export const getUnifiedLivePrompt = (
	taskFrame?: TaskFrame,
	retrievedDocs: DocChunk[] = [],
): string => {
...
export const generateTeammateResponse = async (
	...
	const fullSystemPrompt = buildUnifiedSystemPrompt(
		taskFrame,
		retrievedDocs,
		!!image,
	);
  ...
  `;
	...
	try {
		const response = await requestGemini<{ text?: string }>("teammate", {
			parts,
		});
...
export const generateSpeech = async (
	...
	try {
		const response = await requestGemini<{ audio?: string | null }>("speech", {
			text,
			voice,
		});

		// Return base64 audio data
		return response.audio || undefined;
	} catch (error) {
...
Return JSON only.
  `;
	...
	try {
		const response = await requestGemini<{ text?: string }>("memoryPatch", {
			prompt,
		});
 ...
 */
export const detectSessionIntent = (
	message: string,
): "resume" | "pause" | "clear" | "none" => {
