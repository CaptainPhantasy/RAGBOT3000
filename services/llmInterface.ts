...
type CommandResult = { status: string; result?: any; error?: string };
...
export class LLMInterface {
	...
	public async getLatestObservations(
		verbosity: "minimal" | "standard" | "verbose" = "verbose",
	): Promise<string> {
		...
		if (verbosity === "standard" || verbosity === "verbose") {
			...
			response.accessibility_summary = {
				...
				keyboard_issues:
					latest.accessibility.keyboard_navigation.tab_order_issues.length,
			};
			response.ux_issues =
				latest.ux_assessment.navigation_flow.user_flow_issues;
		}
	...
	private async processCommand(command: LLMCommand): Promise<CommandResult> {
	...
	private async clickElement(selector: string): Promise<CommandResult> {
	...
	private async navigateTo(url: string): Promise<CommandResult> {
		if (!url) {
			return {
				status: "failed",
	...
	private async extractText(selector: string): Promise<CommandResult> {
	...
	private async getElementProperty(
		selector: string,
		property: string,
		...
		const value =
			style.getPropertyValue(property) || (element as any)[property];
	...
	private async scrollToElement(selector: string): Promise<CommandResult> {
		const element = document.querySelector(selector);
		if (!element) {
			return {
				status: "failed",
	...
	private async takeScreenshot(_region?: string): Promise<CommandResult> {
	...
	private async analyzeElement(selector: string): Promise<CommandResult> {
		const element = document.querySelector(selector);
		if (!element) {
			return {
				status: "failed",
