...
function diffSnapshot(): string[] {
	...
	if (current !== lastSnapshot) {
		...
		if (newCount !== oldCount) {
			changes.push(
				`Element count: ${oldCount} → ${newCount} (${newCount - oldCount > 0 ? "+" : ""}${newCount - oldCount})`,
			);
		}
...
function selectorOf(el: Element): string {
	...
	const cls = Array.from(el.classList)
		...
		.slice(0, 2);
	if (cls.length)
		return `${tag}.${cls.map((className) => CSS.escape(className)).join(".")}`;
	if (el.parentElement) {
...
export async function analyzePage(args: {
	include_css?: boolean;
	...
	document.querySelectorAll("form").forEach((form) => {
		...
		form.querySelectorAll("input,select,textarea").forEach((inp) => {
			...
			inputs.push({
				...
				label:
					label?.textContent?.trim() || i.getAttribute("aria-label") || null,
				issue:
					!label &&
	...
	document
		.querySelectorAll(
			'a,button,input[type="submit"],input[type="button"],[role="button"],[tabindex]',
		)
		.forEach((el) => {
	...
	if (!document.documentElement.lang)
		techIssues.push("Missing lang attribute on <html>");
	if (!document.querySelector('meta[name="viewport"]'))
		techIssues.push("Missing viewport meta tag");
	if (!document.querySelector('meta[name="description"]'))
		techIssues.push("Missing meta description");
	...
	score -= result.forms.reduce(
		(acc: number, f: any) =>
			acc + f.inputs.filter((i: any) => i.issue).length * 5,
		0,
	);
...
export async function analyzeElement(args: { selector: string }) {
	...
	return {
		...
		computed_styles: {
			...
			flexDirection: style.display.includes("flex")
				? style.flexDirection
				: undefined,
			gridTemplateColumns: style.display.includes("grid")
...
export async function checkAccessibility(args: {
	scope?: string;
	level?: string;
}) {
	...
	document.querySelectorAll("h1,h2,h3,h4,h5,h6").forEach((h) => {
		...
		if (level > lastLevel + 1 && lastLevel > 0) {
			violations.push({
				rule: "1.3.1 Heading Order",
				severity: "moderate",
				element: selectorOf(h),
				fix: `Heading skips from h${lastLevel} to h${level}`,
			});
		}
		lastLevel = level;
	});
...
export async function extractCSS(args: {
	selector: string;
	properties?: string[];
}) {
	const el = document.querySelector(args.selector);
...
async function openTab(args: Record<string, any>): Promise<any> {
	if (!extensionBridge.isConnected()) {
		return {
			error:
				"Browser extension not installed. Install the Tom The Peep extension for multi-tab control.",
		};
	}
...
export async function extractText(args: { selector: string }) {
	...
	if (elements.length === 0)
		return { error: `No elements found: ${args.selector}` };
...
export async function getPageState() {
	...
	return {
		...
		active_element:
			activeEl && activeEl !== document.body ? selectorOf(activeEl) : null,
...
function analyzeCSSSnapshot() {
	...
	for (const prop of rootStyle) {
		if (prop.startsWith("--")) {
			customProps.push({
				name: prop,
				value: rootStyle.getPropertyValue(prop).trim().slice(0, 80),
			});
		}
	}
	...
	document.querySelectorAll("*").forEach((el) => {
		...
		if (s.display === "flex" || s.display === "inline-flex")
			flexContainers.push(selectorOf(el));
		if (s.display === "grid" || s.display === "inline-grid")
			gridContainers.push(selectorOf(el));
...
function analyzeAccessibilitySnapshot() {
	...
	document.querySelectorAll("img").forEach((img) => {
		if (!(img as HTMLImageElement).alt)
			violations.push({ type: "missing_alt", element: selectorOf(img) });
	});
...
function checkContrastSnapshot(selector?: string) {
	...
	const elements = document.querySelectorAll(
		selector || "p, span, a, button, h1, h2, h3, h4, h5, h6, li, td, th, label",
	);
...
export async function writeObservation(args: { summary?: string }) {
	...
	(analysis.accessibility?.violations || []).forEach((v: any) => {
		md += `- ${v.type}: \`${v.element}\`\n`;
	});
	md += `\n## Contrast Issues (${analysis.contrast_issues?.length || 0})\n`;
	(analysis.contrast_issues || []).forEach((c: any) => {
		md += `- \`${c.element}\` ratio ${c.ratio}:1 (need ${c.required}:1)\n`;
	});
	...
	(analysis.forms || []).forEach((f: any) => {
		...
		f.inputs.forEach((i: any) => {
			md += `  - ${i.type} ${i.name || ""} ${i.issue ? "⚠️ " + i.issue : "✅"}\n`;
		});
	});
	...
	try {
		...
		console.warn(
			"[RAGBOT] Failed to write observation to the local bridge:",
			error,
		);
	}
...
export async function readCommands() {
	...
	try {
		...
		console.warn(
			"[RAGBOT] Failed to read commands from the local bridge; using localStorage:",
			error,
		);
	...
	const pending = commands
		.split("### Command #")
		.slice(1)
		.filter((c) => c.includes("(Pending)"));
	return {
		pending_count: pending.length,
		commands: pending.map((c) => c.slice(0, 200)),
	};
}
...
export async function executeToolCall(name: string, args: any): Promise<any> {
	const extensionPageTools = new Set([
		"analyze_page",
		"analyze_element",
		"find_elements",
		"check_accessibility",
	...
	if (extensionPageTools.has(name) && extensionBridge.isConnected()) {
		return extensionBridge.execute(name as any, args);
	}

	switch (name) {
		case "analyze_page":
			return analyzePage(args);
		...
		case "get_tab_state":
			return getTabState(args);
		case "write_observation":
			return writeObservation(args);
		case "read_commands":
			return readCommands();
		default:
