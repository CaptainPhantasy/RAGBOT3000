...
export async function writeObservation(args: { summary?: string }) {
  ...
  // Store in localStorage (browser) and also try to write to console for external tools
  const existing = localStorage.getItem(OBS_KEY) || '';
...
export async function readCommands() {
  const commands = localStorage.getItem(CMD_KEY) || '';
