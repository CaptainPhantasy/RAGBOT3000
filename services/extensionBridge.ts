...
export class ExtensionBridge {
	...
	private listeningForExtension = false;
	 ...
	 */
	isAvailable(): boolean {
		return Boolean(this.resolveExtensionId());
	}
	...
	connect(): Promise<void> {
		return new Promise<void>((resolve, reject) => {
			...
			this.extensionId = this.resolveExtensionId();

			if (!this.extensionId) {
				reject(
					new Error("Extension ID not found. Is the extension installed?"),
				);
				return;
			}
	...
	listenForExtension(): void {
		if (typeof window === "undefined" || this.listeningForExtension) return;

		this.listeningForExtension = true;
		window.addEventListener("tom-extension-ready", this.handleExtensionReady);
		this.handleExtensionReady();
		window.dispatchEvent(new CustomEvent("tom-extension-discovery"));
	}

	stopListeningForExtension(): void {
		if (typeof window === "undefined" || !this.listeningForExtension) return;
		window.removeEventListener(
			"tom-extension-ready",
			this.handleExtensionReady,
		);
		this.listeningForExtension = false;
	}

	// ---------- private ----------

	private resolveExtensionId(): string | null {
		if (typeof window === "undefined") return null;
		const marker = document.querySelector<HTMLMetaElement>(
			'meta[name="tom-extension-id"]',
		);
		const candidate =
			marker?.content || window.__TOM_EXTENSION_ID__ || this.extensionId;
		return typeof candidate === "string" && candidate.length > 0
			? candidate
			: null;
	}

	private handleExtensionReady = (event?: Event): void => {
		const detail = event instanceof CustomEvent ? event.detail : undefined;
		if (detail && typeof detail.extensionId === "string") {
			this.extensionId = detail.extensionId;
		} else {
			this.extensionId = this.resolveExtensionId();
		}

		if (this.extensionId && !this.isConnected()) {
