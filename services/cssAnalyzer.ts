...
export interface CSSAnalysis {
  ...
  animation_transitions: {
    ...
    transitions: Array<{ property: string; duration: string; elements: string[] }>;
  };
  potential_issues: {
...
export class CSSAnalyzer {
  ...
  private extractComputedStyles() {
    ...
    elements.forEach(element => {
      ...
      const relevantProperties = [
        ...
        'color', 'background', 'font', 'text', 'flex', 'grid', 'transform',
        'transition', 'animation', 'z-index', 'overflow', 'visibility'
      ];
      ...
      if (element.parentElement) {
        ...
        ['font-family', 'font-size', 'line-height', 'color'].forEach(prop => {
          if (style.getPropertyValue(prop) === parentStyle.getPropertyValue(prop)) {
            inherited.push(prop);
          }
        });
      }
  ...
  private analyzeCustomProperties() {
    ...
    document.querySelectorAll('*').forEach(element => {
      ...
      Array.from(style).forEach(prop => {
        if (prop.startsWith('--') && !customProps.find(cp => cp.name === prop)) {
          customProps.push({
            name: prop,
  ...
  private analyzeFlexbox() {
    ...
    const flexItems = flexContainers.flatMap(container => 
      Array.from(container.children).map(child => this.generateDetailedSelector(child))
    );
    ...
    flexContainers.forEach(container => {
      ...
      if (style.flexDirection === 'row' && style.width === '0px') {
        issues.push(`Flex container ${this.generateDetailedSelector(container)} has zero width with row direction`);
      }
    });
  ...
  private analyzeGrid() {
    ...
    gridContainers.forEach(container => {
      const style = getComputedStyle(container);
      if (style.gridTemplateColumns === 'none' && style.gridTemplateRows === 'none') {
        issues.push(`Grid container ${this.generateDetailedSelector(container)} has no explicit tracks defined`);
      }
    });
  ...
  private analyzeAnimationsAndTransitions() {
    const animations: Array<{ name: string; duration: string; elements: string[] }> = [];
    const transitions: Array<{ property: string; duration: string; elements: string[] }> = [];
    ...
    document.querySelectorAll('*').forEach(element => {
      ...
      if (style.animationName !== 'none') {
        const animationNames = style.animationName.split(',').map(n => n.trim());
      ...
      // Extract transitions
      if (style.transitionProperty !== 'all' && style.transitionProperty !== 'none') {
    ...
    elements.forEach(el => {
      ...
      if (style.position === 'absolute' || style.position === 'fixed') {
        ...
        if (parent && getComputedStyle(parent).position === 'static') {
          shifts.push(`${this.generateDetailedSelector(el)} is positioned relative to a static parent`);
        }
      }
    });
    return shifts;
  ...
  private findSpecificityConflicts(): Array<{ selector: string; conflict_type: string }> {
    return []; // Placeholder implementation
  }
  
  private identifyPerformanceIssues(): Array<{ property: string; impact: string }> {
    ...
    document.querySelectorAll('*').forEach(element => {
      ...
      if (style.boxShadow !== 'none' && style.boxShadow !== 'rgba(0, 0, 0, 0)') {
        issues.push({
          property: 'box-shadow',
