...
export class LiveSession {
	...
	async connect(externalStream?: MediaStream, systemInstruction?: string) {
		...
		const memoryContext = activeMemory
			? memoryManager.formatForContext(activeMemory)
...
Keep answers concise and helpful.`;
			...
			const { token, model } = await requestLiveToken(
				finalInstruction,
				getLiveVoice(),
			);
			if (signal.aborted) return;
			...
			const sessionPromise = genAI.live.connect({
				...
				callbacks: {
					onopen: () => {
						...
						this.initAudioInput(
							stream,
							sessionPromise as Promise<GeminiLiveSession>,
						).catch((err: unknown) => {
	...
	public startVideoStream(stream: MediaStream) {
		...
		// Send at most one frame per second, without overlapping sends.
		this.videoInterval = window.setInterval(async () => {
			...
			if (this.videoElement.readyState === this.videoElement.HAVE_ENOUGH_DATA) {
				...
				const base64Data = this.canvasElement
