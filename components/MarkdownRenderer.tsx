  ...
  return (
    <div className="space-y-2 text-sm leading-relaxed">
      {lines.map((line, idx) => {
        if (line.startsWith('# '))
          return (
            <h1 key={idx} className="text-xl font-bold mt-4 mb-2 text-slate-800">
              {line.replace('# ', '')}
            </h1>
          );
        if (line.startsWith('## '))
          return (
            <h2 key={idx} className="text-lg font-semibold mt-3 mb-1 text-slate-700">
              {line.replace('## ', '')}
            </h2>
          );
        ...
        if (line.startsWith('1. '))
          return (
            <li key={idx} className="ml-4 list-decimal text-slate-600">
              {line.replace(/^\d+\. /, '')}
            </li>
          );
