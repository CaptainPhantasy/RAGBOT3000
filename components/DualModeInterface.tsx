...
export const DualModeInterface: React.FC = () => {
	...
	const generateHumanResponse = (query: string, analysis: any): string => {
		...
		if (lowerQuery.includes("accessibility") || lowerQuery.includes("wcag")) {
			const violations =
				analysis.accessibility.wcag_compliance.violations.length;
			const level = analysis.accessibility.wcag_compliance.level;
			return `This page has WCAG ${level} compliance with ${violations} violations. ${
				violations > 0
					? "The main issues are: " +
						analysis.accessibility.wcag_compliance.violations
							.slice(0, 3)
							.map((v: any) => v.description)
							.join(", ")
					: "Great job! No major accessibility issues detected."
			}`;
		}

		if (lowerQuery.includes("css") || lowerQuery.includes("style")) {
			const cssIssues =
				analysis.css_analysis.potential_issues.performance_impact.length;
		...
		// Default response
		return `This page scored ${analysis.summary.overall_score}/100. ${analysis.summary.positive_aspects.join(". ")}. ${
			analysis.summary.recommended_fixes.length > 0
				? "Consider these improvements: " +
					analysis.summary.recommended_fixes
						.slice(0, 2)
						.map((f) => f.issue)
						.join(", ")
				: ""
			...
			<div className="bg-white rounded-lg shadow-lg p-6">
					...
					<div className="flex items-center justify-between">
						...
						{isAnalyzing ? (
							...
							<span className="text-green-600">
								Complete (Score: {analysisResults.summary.overall_score}/100)
							</span>
						) : (
					...
					{analysisResults && (
						<div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
							<div>
								...
								{analysisResults.accessibility.wcag_compliance.level}
							</div>
							<div>
								<span className="font-medium">Issues:</span>{" "}
								{analysisResults.summary.critical_issues.length}
							</div>
							<div>
								<span className="font-medium">Layout:</span>{" "}
								{analysisResults.layout.navigation_elements.length} nav elements
							</div>
							<div>
								<span className="font-medium">CSS Issues:</span>{" "}
								{
									analysisResults.css_analysis.potential_issues
										.performance_impact.length
								}
							</div>
						</div>
					)}
				...
				{(mode === "llm" || mode === "dual") && (
					<div className="mb-6 p-4 bg-purple-50 rounded">
						...
						<div className="mt-3 text-sm text-gray-600">
							<strong>Available commands:</strong> click_element, navigate,
							extract_text, get_element_property, wait_for_element,
							scroll_to_element, analyze_element
						</div>
				...
				{memory && (
					<div className="text-xs text-gray-500">
						Session: {memory.session_id} | Observations:{" "}
						{memory.observations.length} | Last Updated:{" "}
						{new Date(memory.last_updated).toLocaleTimeString()}
					</div>
				)}
			</div>
		</div>
