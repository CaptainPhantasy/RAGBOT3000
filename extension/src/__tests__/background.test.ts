...
const controllerMocks = vi.hoisted(() => ({
	attachToTab: vi.fn().mockResolvedValue(undefined),
	detachFromTab: vi.fn().mockResolvedValue(undefined),
	executeToolCall: vi.fn(async (command: TomCommand, _tabId: number) => ({
		mockResult: true,
		tool: command.tool,
	})),
	onTabClosed: vi.fn(),
...
vi.mock("../keepalive", () => ({
	...
	saveState: controllerMocks.saveState,
}));

// Mock PageController — these tests cover background routing, not Puppeteer.
vi.mock("../pageController", () => ({
	default: class MockPageController {
		attachToTab = controllerMocks.attachToTab;
		detachFromTab = controllerMocks.detachFromTab;
		executeToolCall = controllerMocks.executeToolCall;
		onTabClosed = controllerMocks.onTabClosed;
		onTabUpdated = controllerMocks.onTabUpdated;
	},
}));
...
describe("Tab management — exported functions", () => {
	beforeEach(() => {
		resetChromeAPIs();
		Object.values(controllerMocks).forEach((mock) => mock.mockClear());
	});
	...
	it("openTab creates a new tab and returns TabInfo", async () => {
		...
		expect(controllerMocks.attachToTab).toHaveBeenCalledWith(200);
		expect(controllerMocks.saveState).toHaveBeenCalledWith(
			expect.objectContaining({ controlledTabs: [200] }),
		);
	...
	it("openTab throws when Chrome returns no tab ID", async () => {
		...
		await expect(openTab("about:blank")).rejects.toThrow(
	...
	it("closeTab calls chrome.tabs.remove", async () => {
		...
		expect(controllerMocks.detachFromTab).toHaveBeenCalledWith(200);
	});
	...
	it("switchTab activates a tab and returns TabInfo", async () => {
		...
		expect(controllerMocks.attachToTab).toHaveBeenCalledWith(300);
...
describe("Message routing — onMessageExternal", () => {
	let onMessageExternal: (
		message: unknown,
		sender: object,
		sendResponse: (r: TomResponse) => void,
	) => boolean | void;
	...
	it("rejects messages from Railway wildcard origins", async () => {
		chromeMock.tabs.query.mockResolvedValue([]);
	...
	it("accepts camelCase tabId for tab commands", async () => {
		chromeMock.tabs.update.mockResolvedValue({
			id: 501,
			url: "https://example.com",
			active: true,
		...
		const sendResponse = vi.fn();
		onMessageExternal(
			makeCommand("switch_tab", { tabId: 501 }),
			{ origin: "http://localhost:3000" },
			sendResponse,
		);
		await flushMicrotasks();
		expect(sendResponse.mock.calls[0][0].success).toBe(true);
		expect(chromeMock.tabs.update).toHaveBeenCalledWith(501, { active: true });
	});

	it("attaches the controlled tab before routing page tools", async () => {
		chromeMock.tabs.create.mockResolvedValue({
			id: 502,
			url: "https://example.com",
			active: true,
		...
		controllerMocks.attachToTab.mockClear();
		const sendResponse = vi.fn();
		onMessageExternal(
			makeCommand("analyze_page"),
			{ origin: "http://localhost:3000" },
			sendResponse,
		);
		await flushMicrotasks();
		expect(sendResponse.mock.calls[0][0].success).toBe(true);
		expect(controllerMocks.attachToTab).toHaveBeenCalledWith(502);
		expect(
			controllerMocks.attachToTab.mock.invocationCallOrder[0],
		).toBeLessThan(controllerMocks.executeToolCall.mock.invocationCallOrder[0]);
	});
});
