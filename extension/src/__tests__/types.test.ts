...
import type { TomCommand, TomResponse, TomEvent, TabInfo, ExtensionStatus, ToolName } from '../types';
...
describe('TomResponse', () => {
  it('represents a success response with result payload', () => {
    const response: TomResponse = {
      ...
      requestId: 'req-001',
      success: true,
      result: { tabId: 1, url: 'https://example.com', title: 'Example', active: true, status: 'complete' },
    };
...
describe('ToolName', () => {
  it('covers all 23 tool names without duplicates', () => {
    const allTools: ToolName[] = [
      // Original 13
      'analyze_page', 'analyze_element', 'find_elements', 'check_accessibility',
      'extract_css', 'check_contrast', 'click_element', 'scroll_to',
      'navigate_to', 'extract_text', 'get_page_state', 'write_observation',
      'read_commands',
      // New 10
      'open_tab', 'close_tab', 'switch_tab', 'list_tabs',
      'type_text', 'fill_form', 'select_option', 'take_screenshot',
      'wait_for_element', 'get_tab_state',
    ];
...
describe('ExtensionStatus', () => {
  it('accepts all valid status values', () => {
    const statuses: ExtensionStatus[] = ['ready', 'busy', 'error', 'disconnected'];
