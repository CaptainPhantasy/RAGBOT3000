...
import { saveState, loadState, initKeepalive } from "../keepalive";
...
describe("initKeepalive", () => {
	...
	it("registers alarm, startup, and installed listeners", async () => {
		...
		expect(chromeMock.runtime.onStartup.getListeners().length).toBeGreaterThan(
			0,
		);
		expect(
			chromeMock.runtime.onInstalled.getListeners().length,
		).toBeGreaterThan(0);
	});
