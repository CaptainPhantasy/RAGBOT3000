...
export const useDragAndResize = () => {
			...
			if (handle.includes("corner")) {
				...
				if (absDeltaX > absDeltaY) {
					if (handle.includes("right"))
						...
						newX =
							resizeStateRef.current.startPosX +
							(resizeStateRef.current.startWidth - newWidth);
					}
					newHeight = newWidth / aspectRatio;
					if (handle.includes("top"))
						newY =
							resizeStateRef.current.startPosY +
							(resizeStateRef.current.startHeight - newHeight);
				} else {
				...
				if (handle === "right")
					newWidth = Math.max(
						...
						Math.min(
							resizeStateRef.current.startWidth + deltaX,
							vw - resizeStateRef.current.startPosX,
						),
					);
				else if (handle === "left") {
					...
					newHeight = Math.max(
						...
						Math.min(
							resizeStateRef.current.startHeight + deltaY,
							vh - resizeStateRef.current.startPosY,
						),
					);
				else if (handle === "top") {
	...
	const handleResizeStart = useCallback(
		(e: ReactMouseEvent, handle: string) => {
			resizeStateRef.current = {
				isActive: true,
				handle,
				startX: e.clientX,
