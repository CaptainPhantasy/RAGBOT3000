	...
	const handleModeChange = useCallback(
		async (mode: InteractionMode) => {
			...
			if (!ready) {
				if (isIOS()) {
					setLastError(
						"Microphone access required. Please allow access when prompted, or check Settings > Safari > Microphone.",
					);
				} else {
					setLastError(
	...
	useEffect(() => {
		...
		const tick = () => {
			...
			if (!agentSpeaking) {
				...
				vadNoiseFloorRef.current = Math.max(
					0.01,
					vadNoiseFloorRef.current * 0.98 + rms * 0.02,
				);
